...
(defvar anything-source-mljh-file-search
  '((name . "麻辣江湖文件列表")
	...
	(real-to-display . (lambda(s)
						 (let (prefix)
						   (if (string-match "gameserver" s) (setq prefix "GS-: "))
						   (if (string-match "gamecenter" s) (setq prefix "GC+: "))
						   (if (string-match "client" s) (setq prefix "-UC: "))
						   (if (string-match "client/ui" s) (setq prefix "-UI: "))
						   (if (string-match "[^/]*/[^/]*$" s)
							   (concat prefix  (match-string 0 s))
							 (s))
						   )
						 ))
    (requires-pattern . 2)
